    ...
    <section>
        <div>
            <header>
                ...
                <div class="hamburger-menu-button" :class="{'open': showMenu}" @click="toggleMenu">
                    <span class="hamburger-menu-button-burger" :class="{'open': showMenu}"></span>
                </div>
                <nav :class="{'open': showMenu}">
                    <div class="menu-nav" :class="{'open': showMenu}">
                        <router-link v-for="route in routes" :to="route.path" class="menu-nav-item" :class="{'open': showMenu}">{{ `${route.name[0].toLocaleUpperCase() + route.name.substring(1)}` }}</router-link>
                    </div>
                </nav>
